...
function printServicesList() {
    var services = notify.getServices();

    _.chain(services)
        .map(function(serv, name) {
            return {
                name: name,
        ...
        .sortBy('name')
        .each(function(serv) {
            console.log(sprintf('%-12s %s', serv.name, serv.descr));
            console.log();

            if (_.isEmpty(serv.config)) {
                console.log(sprintf('  No options.', ''));
            }
            else {
                console.log(sprintf('  Options:', ''));

                var opts = _.map(serv.config, function(descr, key) {
                    return {
                        name: '--'+serv.name+'-'+_.kebabCase(key),
                        descr: descr
                    };
                });

                _.each(opts, function(opt) {
                    console.log(sprintf('    %-20s  %s', opt.name, opt.descr));
                })
            }
...
/* obtain the list of services to use specified in the command arguments.
...
function getServiceOptions(argv, servName) {
    var config = notify.getService(servName).config,
        prefix = servName+'-',
        opts = {};
...
if (!action || action === 'msg' || action === 'run') {
    ...
    _.each(services, function(servName) {
        ...
        var servConfig = _.extend({},
            ...
            userConfig.services && userConfig.services[servName] || {},
    ...
    // tnotify, run it and capture its output
    if (action === 'run') {
        ...
        // ensure the command name is separated correctly from its arguments
        var cmd = actionArgs.join(' ').trim().split(' ');

        var title = argv.title || cmd.join(' '),
            cmdName = cmd[0],
            cmdArgs = cmd.splice(1);
    ...
    if (actionArgs.length && actionArgs[0] === 'list') {
        console.log(JSON.stringify(userConfig, null, '  '));
        process.exit();
    }
    ...
    _.each(services, function(servName) {
        var servConfig = notify.getService(servName).config;
        ...
        _.each(servConfig, function(description, varName) {
            queue.push(function(next) {
                ...
                var defaultVal = userConfig.services
                    && userConfig.services[servName]
                    && userConfig.services[servName][varName] || '';
                ...
                read(readOpts, function(err, line) {
                    ...
                    if (!userConfig.services[servName])
                        userConfig.services[servName] = {};
                    ...
                    userConfig.services[servName][varName] = value;
                    next();
                });
            });
        });
